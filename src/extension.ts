...
export function activate(context: vscode.ExtensionContext) {
    ...
    let openSettingsCommand = vscode.commands.registerCommand('slurm-vscode.openSettings', () => {
        vscode.commands.executeCommand('workbench.action.openSettings', '@ext:slurm-vscode');
    });
    ...
    let selectAllCommand = vscode.commands.registerCommand('slurm-vscode.selectAll', () => {
        jobsWebviewProvider.selectAll();
    });

    // Register clear selection command
    let clearSelectionCommand = vscode.commands.registerCommand('slurm-vscode.clearSelection', () => {
        jobsWebviewProvider.clearSelection();
    });

    // Register refresh command
    let refreshCommand = vscode.commands.registerCommand('slurm-vscode.refresh', () => {
        slurmJobProvider.refresh();
    });

    // Register a command to open the job manager
    let openManagerCommand = vscode.commands.registerCommand('slurm-vscode.openJobManager', () => {
        vscode.window.showInformationMessage('Slurm Job Manager is now active!');
    });

    // Register command to kill a job
    let killJobCommand = vscode.commands.registerCommand('slurm-vscode.killJob', async (job: JobItem) => {
        // Set the job to confirm state
        slurmJobProvider.setJobToConfirm(job.jobId);
    });

    // Register command to confirm kill
    let confirmKillCommand = vscode.commands.registerCommand('slurm-vscode.confirmKill', async (job: JobItem) => {
        try {
    ...
    let cancelKillCommand = vscode.commands.registerCommand('slurm-vscode.cancelKill', async (job: JobItem) => {
        slurmJobProvider.clearJobToConfirm(job.jobId);
    });
